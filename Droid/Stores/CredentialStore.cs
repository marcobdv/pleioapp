...
namespace Pleioapp.Droid
{
	public class CredentialStore : ITokenStore
	{
		...
		public CredentialStore() {
			store = AccountStore.Create(Forms.Context);
		...
		public void clearTokens() {
			var accounts = AccountStore.Create(Forms.Context).FindAccountsForService(context).ToList();
			accounts.ForEach(account => AccountStore.Create(Forms.Context).Delete(account, context));
		}
	}
}